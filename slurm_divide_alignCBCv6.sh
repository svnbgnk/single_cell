...
#  echo "nocDNA"
  ...
  if [[ "$CBC_only" == "true" ]]; then
    /scratch/sboenigk/executables/single_cell_scripts_exe/splitReads -f "${tdir}/P1.fasta" -o "${tdir}/P1_split" -l 25
  else
    echo "CBC + UMI extracted"
    /scratch/sboenigk/executables/single_cell_scripts_exe/splitReads -f "${tdir}/P1.fasta" -o "${tdir}/P1_split" -l 35
  fi
  ...
  if [ $sizel -ne 0 ]; then
    if [[ "$CBC_only" == "true" ]]; then
      /scratch/sboenigk/executables/lastest_flexbar/flexbar -r "${tdir}/P1_split_left_tail_trimmed.fasta" -t $LEFT -a $CBC_UMI_NAMES -n 40 -N 20 -g -ae LTAIL -ao 10 -l ALL -eve -ag -1 -ai -1 -at 0.24
    else
      /scratch/sboenigk/executables/lastest_flexbar/flexbar -r "${tdir}/P1_split_left_tail_trimmed.fasta" -t $LEFT -a $CBC_UMI_NAMES -n 40 -N 2 -g -ae LTAIL -ao 20 -l ALL -eve -ag -1 -ai -1 -at 0.24
    fi
  fi
  date
  ...
  if [ $sizer -ne 0 ]; then
    if [[ "$CBC_only" == "true" ]]; then
      /scratch/sboenigk/executables/lastest_flexbar/flexbar -r "${tdir}/P1_split_right_tail_trimmed.fasta" -t $RIGHT -a $CBC_UMI_NAMES -n 40 -N 20 -g -ae RTAIL -ac -ao 10 -l ALL -eve -ag -1 -ai -1 -at 0.24
    else
      /scratch/sboenigk/executables/lastest_flexbar/flexbar -r "${tdir}/P1_split_right_tail_trimmed.fasta" -t $RIGHT -a $CBC_UMI_NAMES -n 40 -N 2 -g -ae RTAIL -ac -ao 20 -l ALL -eve -ag -1 -ai -1 -at 0.24
    fi
  fi
  date
