...
echo "P1_left $sizel"
if [ $sizel -ne 0 ]; then
  executables/flexbar -r "${tdir}/P1_split_left_tail_trimmed.fasta" -t $LEFT -a $CBC -n 40 -N 20 -g -at LTAIL -ao 10 -l ALL -alt -ag -1 -ai -1 -ae 0.24
fi
date
sizer=$(wc -l ${tdir}/P1_split_right_tail_trimmed.fasta | cut -d " " -f 1)
echo "P1right $sizer"
if [ $sizer -ne 0 ]; then
  executables/flexbar -r "${tdir}/P1_split_right_tail_trimmed.fasta" -t $RIGHT -a $CBC -n 40 -N 20 -g -at RTAIL -ac ONLY -ao 10 -l ALL -alt -ag -1 -ai -1 -ae 0.24
fi
date
#  echo "Copied Logs"
cat "${LEFT}.log" >> $leftT
